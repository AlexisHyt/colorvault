...
export function deriveHarmonies(primaryHex: string): Record<Exclude<ColorRole, "primary">, string> {
...
export function generateWebPalette(colors: Record<ColorRole, string>): WebPalette {
  return {
...
export function exportAsCSSVariables(palette: WebPalette): string {
  const lines = [":root {"];
  for (const [role, scale] of Object.entries(palette) as [ColorRole, PaletteScale][]) {
    lines.push(`  /* ${role} */`);
