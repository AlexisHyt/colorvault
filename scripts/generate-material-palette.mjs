/**
 * generate-material-palette.mjs
 *
 * Converts Material Design hex colors to oklcha format and outputs
 * a palette JSON compatible with insert-palette.mjs.
 *
 * Usage:
 *   node scripts/generate-material-palette.mjs > scripts/palettes/palette-material.json
 */

// ─── Conversion helpers ──────────────────────────────────────────────────────

function hexToOklcha(hex) {
  hex = hex.replace("#", "");
  ...
  return `oklcha(${fmt(L)}, ${fmt(C)}, ${fmt(H)}, 1)`;
}

// ─── Material Design color data (hex) ────────────────────────────────────────
...
const materialColors = {
  ...
  yellow: {
    ...
    400: "#FFEE58", 500: "#FFEB3B", 600: "#FDD835", 700: "#F9A825",
    800: "#F57F17", 900: "#F57F17",
...
const shadeOrder = [50, 100, 200, 300, 400, 500, 600, 700, 800, 900, "a100", "a200", "a400", "a700"];

const rows = Object.entries(materialColors).map(([colorName, shades]) => {
  const colors = shadeOrder
    .filter((shade) => shades[shade] !== undefined)
    .map((shade) => {
      const shadeSuffix = String(shade).toUpperCase().replace("A", "A");
      // Format shade name: "red-50", "red-A100", etc.
      const shadeLabel =
        typeof shade === "string"
          ? `${colorName}-${shade.toUpperCase()}`
          : `${colorName}-${shade}`;
      return {
        name: shadeLabel,
        color: hexToOklcha(shades[shade]),
      };
    });
...
const palette = {
  palette: {
    ...
    description: "Material Design color palette (all color families with accent variants)",
    category: "main",
  },
  rows,
};
...
const outPath = join(__dirname2, "palettes", "palette-material.json");
